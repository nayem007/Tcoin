...
**Good name must be unique.** Check uniqueness with [google](http://google.com) and [Map of Coins](Tcoin.com) or any other similar service.
...
Example: 
```
const char CRYPTONOTE_NAME[] = "Tcoin";
```

**2. in src/CMakeList.txt file** - set_property(TARGET daemon PROPERTY OUTPUT_NAME "Tcoin**d**")

Example: 
```
set_property(TARGET daemon PROPERTY OUTPUT_NAME "Tcoin")
```
